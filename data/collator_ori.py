...
def collator(items, feature, shuffle=False, perturb=False):
    # Get dataloader batches, pack feature inside
    batch_list = []
    for item in items:
        for x in item:
            batch_list.append((x[0], x[1], x[2][0])) # x[2][0] because one node denotes one sequence, the first label is the trained node's label
            # print(x[2][:-1].shape)
            # exit(0)
            # batch_list.append((x[0], x[1], x[2][:-1]))
    if shuffle:
