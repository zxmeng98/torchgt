...
def collator_malnet(items, max_node=512, multi_hop_max_dist=20, spatial_pos_max=20, myargs=None):
    t1 = time.time()
    ...
    seq_parallel_world_size = myargs.sequence_parallel_size
    ...
    if myargs.model == "graphormer":
        ...
        num_virtual_tokens = 1
        edge_indexes_raw = [fix_edge_index(edge_indexes_raw[idx], num_nodes[idx]) for idx in range(len(edge_indexes_raw))]
